...
function formIframeDidLoad() {
	...
	if (i < y.length - 1 && i < endingCount) {
		...
		if (links == null || links.length != 1) {
			...
			if (onlyPostInvalidLinks == true) {
				//check the link first
				var soundCloudiFrame = document.createElement("iframe");
				soundCloudiFrame.id  = 'soundCloudiFrame' + i;
				soundCloudiFrame.src = "https://w.soundcloud.com/player/?url=" + link;
				console.log("Checking " + i + " Link Being checked is " + link);



//https://w.soundcloud.com/player/?url=https%3A//api.soundcloud.com/tracks/271323509&amp;auto_play=false&amp;hide_related=false&amp;show_comments=true&amp;show_user=true&amp;show_reposts=false&amp;visual=true

//"https://w.soundcloud.com/player/?url=https%3A//api.soundcloud.com/tracks/221873974"

//https://soundcloud.com/evangartner/evan-gartner-chelsea-cutler-going-home-again

//if iframe contains class title__h1, we know its valid. if not, then it isnt

				// soundCloudiFrame.onLoad = function() {
				// 	soundcloudIframeDidLoad(name, link, i);
				// }

   

				document.getElementById("u_0_p").appendChild(soundCloudiFrame);

				var loadedSCiframe = document.getElementById('soundCloudiFrame' + i);
				var sc = SC.Widget(loadedSCiframe);
